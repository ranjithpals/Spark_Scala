...
object internalsOfJoin extends App {
  ...
  .load()
  /*
  //Join the orders with customers using outer join to show only customers who have placed an order
  val joinType1 = "outer"
  val joinCondition1 = ordersDF.col("order_customer_id") === customersDF.col("customer_id")
  val outerJoin = ordersDF.join(customersDF, joinCondition1, joinType1)
  outerJoin.show()
  */
  ...
  val innerJoin = ordersDF.join(customersDF, joinCondition2, joinType2).sort("order_id")
  innerJoin.show(100)
